import { RegistrationStatus } from "../../Models/registrations.type.ts";
import { useState } from "react";
import ConfirmModal from "../../components/common/ConfirmModal.tsx";
import { Link } from "react-router";
import { useAuth } from "../../hooks/useAuth.ts";
import { useMyRegistrations } from "../../hooks/useMyRegistrations.ts";
...
const MyRegistrations = () => {
    ...
    const handleDeleteConfirm = async () => {
        ...
        const success = await deleteRegistration(itemToDelete);
        if (success) {
        ...
        <div className="min-h-screen bg-[#0a0f0e] relative overflow-hidden text-white font-black italic uppercase px-6 md:px-12 pt-32 pb-20">
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
                ...
                <div className="space-y-6">
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-32 bg-white/2 border border-white/5 border-dashed rounded-sm">
                            ...
                            <div className="mt-8 text-white/40 font-black uppercase tracking-[0.4em] text-[10px] animate-pulse">Synchronizing_Archive_Data</div>
                        </div>
                    ) : registrations.length > 0 ? (
                        registrations.map((reg) => (
                            <div key={reg.id} className="grid grid-cols-1 md:grid-cols-5 items-center bg-white/2 border border-white/5 p-8 gap-6 hover:bg-white/5 hover:border-accent/40 transition-all group relative overflow-hidden shadow-2xl backdrop-blur-sm">
                                <div className="md:col-span-2 space-y-2">
                                    ...
                                    <p className="text-[10px] text-white/30 not-italic font-bold tracking-widest uppercase">Sector: {reg.race?.location}</p>
                                </div>

                                <div className="flex flex-col">
                                    <span className="text-[9px] text-white/20 md:hidden mb-2 font-bold uppercase tracking-widest italic">Bib Number</span>
                                    <span className="text-4xl font-black tracking-widest text-white group-hover:scale-105 transition-transform origin-left">#{reg.bibNumber}</span>
                                </div>
                                    ...
                                    <div className={`inline-flex items-center gap-3 px-4 py-1.5 border w-fit ${
                                        ...
                                        <span className={`w-2 h-2 rounded-full ${
                                            reg.status === RegistrationStatus.Confirmed ? 'bg-green-500' :
                                                reg.status === RegistrationStatus.Pending ? 'bg-orange-500 animate-pulse' : 'bg-red-500'
                                        }`}></span>
                ...
                <ConfirmModal
                    ...
                    onClose={() => { setIsDeleteModalOpen(false); setError(null); }}
